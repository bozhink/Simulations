...
FPUModel::FPUModel(int _nParticles, int _modeInit, 
        double _amplitude, double _dt, double _tMax,
        int _nModes, 
...
void FPUModel::Run() {
    ...
    while(t < tMax) {
        ...
        if (tTest <= 0) {
            ...
            fXMode << endl;
            /*
             * Writing information ablout the force to file
             */
            X = x[i+1]-x[i];
            Y = x[i]-x[i-1];
            fForce << t << '\t' << X-Y // Linear part of the force
                    << '\t' << alpha*(X*X-Y*Y) // Cuadratic non-linearity
